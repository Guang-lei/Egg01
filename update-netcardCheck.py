...
# SCRIPT_UUID = SC20240603203910017699879
...
def _get_port_id(token, vpc_url, project_id, server_id):
    ...
    if int(resp.status_code) == 200:
        ...
        for i in range(0, len(result["ports"])):
            ...
            if result["ports"][i]["fixed_ips"]:
                _update_allow_ip(token, vpc_url, project_id, port_id)
    else:
...
def _update_allow_ip(token, vpc_url, project_id, port_id):
    body = {
        "port": {
...
if __name__ == "__main__":
    ...
    _get_port_id(iam_token, vpc_url, projectId, serverId)
